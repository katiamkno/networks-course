...
#### Решение
Максимально в сети может быть одновременно 20 пользователей (это пропускная способность разделить на скорость для каждого). То есть нужно найти вероятность $P(12 \leq n \leq 20)$, где $P(n = k)$ имеет биномиальное распределение Вероятность этого примерно 0.56.
