...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ICMP работает на сетевом уровне, а не транспортном
   
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - тип 8, код 0
   - вот такие поля есть: тип (1 байт), код (1 байт), контрольная сумма (2 байта), идентификатор (2 байта), номер последовательности (2 байта) + данные (у меня 32 байта)
![](images/ping_3.png)
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - тип 0, код 0
   - все поля аналогичны 3 и имеют такой же размер
   ![](images/ping_4.png)
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - практически не отличается, поля такие же, только размер данных другой
![](images/trace_1.png)
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Внутренний IP-заголовок + ICMP (оригинальный пакет).
Это позволяет источнику понять, какой именно пакет вызвал ошибку.
   ![](images/trace_2.png)
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Echo Reply содержит данные и идентификаторы для сверки с запросом, а ICMP-ошибки содержат заголовок исходного пакета
   - связано с тем, что Echo используется для измерения задержки и проверки доступности, а ICMP-ошибки — для диагностики проблем
   ![](images/trace_3.png)
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - скачок с 15 на 16
   - имен нет, определение адресов IP говорит, что это скачок из Тюмени в Дубай
![](images/trace_0.png)
## Программирование.
