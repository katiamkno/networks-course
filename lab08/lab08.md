...
Пусть $W$ — максимальный размер окна перегрузки (в байтах), $MSS$ — размер одного сегмента.
В режиме congestion avoidance TCP увеличивает окно на 1 MSS за каждый RTT.

Следовательно, прирост окна за один RTT равен:
$T = \frac{W}{2MSS}$

Окно растёт линейно от $\frac{W}{2}$ до $W$, поэтому среднее значение:
$W_{avg} = \frac{\frac{W}{2} + W}{2} = \frac{3W}{4}$


Средняя пропускная способность равна среднему окну, делённому на RTT:
$R_{avg} = \frac{3W}{4 \cdot RTT}$

$T = \frac{1}{2MSS} \cdot \frac{4 R_{avg} RTT}{3}$


При фиксированных значениях RTT и MSS:
$T = f(R_{avg})$
### Задача 2 (3 балла)
...
#### Решение
Число шагов до W:
$\frac{W}{2}\cdot(1+\alpha)^T = W$

$(1+\alpha)^T = 2$

$T = log_{1+\alpha} 2 = const$ от W

Суммарно сегментов за цикл:
$Cnt = \frac{W}{2}\cdot \sum_{i=0}^{T} (1+\alpha)^i = \frac{W}{2}\cdot \frac{(1+\alpha)^{T+1} - 1}{\alpha}$

Так как $(1+\alpha)^T = 2$, то:

$Cnt = \frac{W}{2}\cdot \frac{(1+\alpha) \cdot 2 - 1}{α} = \frac{W}{2} \cdot \frac{1 + 2α}{α}$

$L = 1/Cnt = \frac{2\alpha}{W(1+2\alpha)}$
