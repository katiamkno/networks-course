...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 0 или 755822348 (raw)
   - 1 или 677225461 (raw)
   - следующий номер после порядкового текущего
   - стоят флаги соответствующие
   ![](images/task_4.png)
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - кажется этот TCP сегмент где-то зашифрован, так как среди сегментов с нужным портом (таким же как в 3 и 4 номерах) его нет
   ![](images/task_5.png)
   Остальное тоже не сделать (
...
#### Демонстрация работы
2 клиента параллельно:
![](images/D_client1.png)
![](images/D_client_2.png)
Сервер:
![](images/D_server.png)
