...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - Header = 26 байт
   - Всего 56 байт, значит полезных 30
    ![](images/3.png)
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Source Address, Destination Address, Header Length, Total Length, Protocol
      - эти поля не должны меняться, они определяют параметры запуска
      - Identification — каждому пакету присваивается уникальный ID, TTL — увеличивается, Header Checksum — пересчитывается при изменении любого поля заголовка
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - TTL всё время 64, Identification меняется
   ![](images/6-1.png)
   ![](images/6-2.png)
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
        - Total Length - последний — меньше (остаток)
        - Flags (More Fragments) - установлен в 1 у всех фрагментов, кроме последнего
        - Fragment Offset
        - Header Checksum
      ![](images/8-2-1.png)
      ![](images/8-2-2.png)
